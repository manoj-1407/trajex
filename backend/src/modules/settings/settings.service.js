...
async function getSettings(businessId) {
    const result = await db.query('SELECT id, name, slug, logo_url, accent_color, sla_default_minutes, timezone, webhook_url, webhook_secret, created_at FROM businesses WHERE id = $1', [businessId]);
...
async function updateSettings(businessId, data) {
    ...
    const result = await db.query(
        'UPDATE businesses SET name = COALESCE($1,name), logo_url = COALESCE($2,logo_url), accent_color = COALESCE($3,accent_color), sla_default_minutes = COALESCE($4,sla_default_minutes), timezone = COALESCE($5,timezone), webhook_url = COALESCE($6,webhook_url), webhook_secret = COALESCE($7,webhook_secret) WHERE id = $8 RETURNING id, name, slug, logo_url, accent_color, sla_default_minutes, timezone, webhook_url, webhook_secret',
