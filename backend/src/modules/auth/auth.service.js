...
async function googleLogin(req, res, next) {
    try {
        const profile = req.body.profile; // Profile from client or passport
        if (!profile) return res.status(400).json({ error: 'Missing profile data' });

        const user = await googleService.handleGoogleLogin(profile, req);
        if (!user.is_active && user.id) return res.status(403).json({ error: 'Account disabled' });
...
module.exports = { register, login, refresh, logout, forgotPassword, resetPassword, googleLogin };
