...
export default function Analytics() {
    ...
    const fetchData = async () => {
      ...
      try {
        const [dashRes, trendRes, timeRes, ridersRes] = await Promise.all([
          ...
          api.get(`/analytics/orders/trend?days=${days}`),
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div style={{ background: 'var(--bg-elevated)', padding: '12px', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow)' }}>
          <p style={{ margin: '0 0 8px 0', fontSize: '12px', fontWeight: 600, color: 'var(--text-muted)' }}>{label}</p>
          {payload.map(p => (
            <p key={p.dataKey} style={{ margin: '0 0 4px 0', fontSize: '13px', color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ display: 'inline-block', width: '8px', height: '8px', borderRadius: '50%', background: p.color || p.fill }} />
              <span style={{ color: 'var(--text-secondary)' }}>{p.name}:</span> <span style={{ fontWeight: 600, fontFamily: 'var(--font-mono)' }}>{p.value}</span>
            </p>
          ))}
        </div>
      );
