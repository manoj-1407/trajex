...
export default function Onboarding() {
    ...
    return (
      <div className="fade-in-up">
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <h2 style={{ fontSize: '24px', fontWeight: 700 }}>Invite your first rider</h2>
        </div>
        ...
        <form onSubmit={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '24px' }}>
          ...
          <Input label="Email Address (Required for Invite)" type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})} disabled={loading} required />
          <Button type="submit" size="lg" loading={loading} fullWidth>Invite Rider →</Button>
        </form>
  ...
  const Step3 = () => {
    ...
    const onSubmit = async (e) => {
      ...
      setLoading(true);
      try {
        const phone = form.customerPhone.startsWith('+') ? form.customerPhone : `+91${form.customerPhone}`;
        await api.post('/orders', { 
          customerName: form.customerName,
          customerPhone: phone,
          dropAddress: form.deliveryAddress,
          pickupLat: 17.3850,  // Hyderabad Base
          pickupLng: 78.4867,
          dropLat: 17.4000,
          dropLng: 78.5000
        });
