...
export default function RiderView() {
  ...
  useEffect(() => {
    ...
    if (online && navigator.geolocation) {
      ...
      api.patch('/auth/profile', { status: 'available' }).catch(()=>{});
      requestWakeLock();
      ...
      watchId = navigator.geolocation.watchPosition(
        (pos) => {
          ...
          if (socket) {
             socket.emit('rider-location', { riderId: user.id, lat: latitude, lng: longitude });
          }
        },
      ...
      if (online === false && navigator.geolocation && watchId) {
        api.patch('/auth/profile', { status: 'offline' }).catch(()=>{});
      }
